...
abstract class AdminModel extends FormModel
{
	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		while (!empty($pks))
		{
			...
			$asset = isset($contexts[$pk]) ? $contexts[$pk] : null;

			if (!$this->table->hasField($this->table->getColumnAlias('asset_id')) && isset($this->table->catid))
			{
				$asset = $this->option . '.category.' . (int) $this->table->catid;
			}

			if (!$asset || !$this->user->authorise('core.edit', $asset))
			{
				$this->setError(\JText::_('JLIB_APPLICATION_ERROR_BATCH_CANNOT_EDIT'));
