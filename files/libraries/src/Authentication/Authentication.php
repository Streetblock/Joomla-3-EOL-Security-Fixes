...
class Authentication extends \JObject
{
	...
	{
		// Optional branch experiment. Preserve the stock path unless explicitly enabled.
		if (\JFactory::getConfig()->get('eol_login_throttle', false)
			&& array_key_exists('password', $credentials))
		{
			return LoginThrottle::authenticate($credentials, $options, function () use ($credentials, $options) {
				return $this->authenticateWithoutThrottle($credentials, $options);
			});
		}
	...
	/** Execute the original plugin chain only after admission to password authentication. */
	private function authenticateWithoutThrottle($credentials, $options)
	{
		// Get plugins
