...
/**
 * Shared depth and collection-alias limits, adapted from Symfony 5.4 security fixes.
 * Kept in this existing file because the hotfix installer replaces existing files only.
 * @internal
 */
final class JoomlaEolParserState
{
    private $depth = 0;
    private $collectionAliases = 0;

    public function enterNestingLevel()
    {
        if (++$this->depth > 128) {
            --$this->depth;
            throw new ParseException('Maximum YAML nesting depth of 128 exceeded.');
        }
    }

    public function leaveNestingLevel()
    {
        if ($this->depth > 0) { --$this->depth; }
    }

    public function countAlias($value)
    {
        if ((\is_array($value) || $value instanceof \stdClass) && ++$this->collectionAliases > 128) {
            throw new ParseException('Maximum number of YAML collection aliases (128) exceeded.');
        }
    }
}
...
class Parser
{
    ...
    /** Internal shared limits for the 2.8 backport; no additional autoloaded file. */
    public static function createState()
    {
        return new JoomlaEolParserState();
    }
    ...
    private function doParse($value, $exceptionOnInvalidType = false, $objectSupport = false, $objectForMap = false)
    {
        ...
        while ($this->moveToNextLine()) {
            ...
            if (self::preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+))?$#u', rtrim($this->currentLine), $values)) {
                ...
                Inline::parse(null, $exceptionOnInvalidType, $objectSupport, $objectForMap, $this->refs, $this->state);
                try {
                ...
                if ('<<' === $key && (!isset($values['value']) || !self::preg_match('#^&(?P<ref>[^ ]+)#u', $values['value'], $refMatches))) {
                    ...
                    if (isset($values['value']) && 0 === strpos($values['value'], '*')) {
                        ...
                        $this->state->countAlias($refValue);
                ...
                if (\is_string($value) && $this->lines[0] === trim($value)) {
                    try {
                        $value = Inline::parse($this->lines[0], $exceptionOnInvalidType, $objectSupport, $objectForMap, $this->refs, $this->state);
                    } catch (ParseException $e) {
                        $e->setParsedLine($this->getRealCurrentLineNb() + 1);
                        $e->setSnippet($this->currentLine);
    ...
    private function parseBlock($offset, $yaml, $exceptionOnInvalidType, $objectSupport, $objectForMap)
    {
        ...
        $this->state->enterNestingLevel();
        try {
            $result = $parser->doParse($yaml, $exceptionOnInvalidType, $objectSupport, $objectForMap);
        } catch (\Exception $e) {
            $this->state->leaveNestingLevel();
            throw $e;
        } catch (\Throwable $e) {
            $this->state->leaveNestingLevel();
            throw $e;
    ...
    private function parseValue($value, $exceptionOnInvalidType, $objectSupport, $objectForMap, $context)
    {
        if (0 === strpos($value, '*')) {
            ...
            $this->state->countAlias($this->refs[$value]);
        ...
        try {
            $parsedValue = Inline::parse($value, $exceptionOnInvalidType, $objectSupport, $objectForMap, $this->refs, $this->state);
